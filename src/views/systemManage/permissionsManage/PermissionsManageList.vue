...
import { getPubAdminPermissionsListAll, postPubAdminPermissionsSaveBatch, deletePubAdminPermissionsDelete } from '@/api/permissions'
import { getPubAdminRouteListAll } from '@/api/route'

export default {
  data () {
    const routeCodesEditRender = {
      name: 'VxeTreeSelect',
      options: [],
      optionProps: {
        value: 'code',
    ...
    return {
      routeCodesEditRender,
      gridOptions
    }
  },
  created () {
    getPubAdminRouteListAll({}).then(res => {
      this.routeCodesEditRender.options = res.data
    })
  },
  methods: {
    async removeRow (row) {
